...
func test_on_player_damage_received(params = use_parameters(hit_params)):
	# given
	var pushback_velocity = params[0]
	var expected_multiplier = params[1]
	# when
	particles._on_player_damage_received(10.0, 20.0, pushback_velocity)
	# then
...
# _get_multiplier pretty much tested in test_on_player_damage_received
